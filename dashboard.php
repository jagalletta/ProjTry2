    ...
    if (mysqli_stmt_execute($stmt)) {
      ...
      if (mysqli_stmt_num_rows($stmt) > 0) {
        ...
        while($row = mysqli_stmt_fetch($stmt)){
          ...
          echo "<h1> Because you searched for " . $searchParameter .":</h1></br>";
//----results for each parameter
            ...
            foreach ($artists->artist as $artist) {
              if ($newCounter <= 3){
                foreach ($artist->image as $image) {
                  if ($image->{'size'} == "large") {
                    echo "<td class='special'><img src='" . $image->{'#text'} . "'></br>";
                  }

                }
                echo "<button type='button' class='btn btn-default btn-lg' onclick='window.open(\"" . $artist->url . "\");' >".$artist->name."</button>";
                echo "</td>";
                $newCounter ++;
              }
            }
          ...
          //and output html formatted response
          // if ($count == 0) {
          //     echo "<h2 class='history-none'> You currently have not searched for any artist. Please search for one and it will be displayed here.</h2>";
          //   }else if(!empty($row['searchParameter']) && count > 0){
          //
          //         $sql = "SELECT Top 3 searchParameter FROM search WHERE userName = ? ORDER By created_at desc limit 3";
          //         if ($stmt = mysqli_prepare($link, $sql)) {
          //             mysqli_stmt_bind_param($stmt, "s", $param_searchParameter);
          //
          //
          //
          //             $param_searchParameter = $query['searchParameter'];
          ...
          //     }
          //
          //     echo " similar to " . $query['searchParameter'] . ".";
          //     echo "<table class='special' style='margin-left:auto; margin-right:auto; margin-bottom:15px'>";
